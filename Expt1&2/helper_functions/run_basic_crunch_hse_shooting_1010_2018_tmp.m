function [dat,info]=run_basic_crunch_hse_shooting_1010_2018_tmp(dir1)
                        ...
                        if q.tgt(kn1,7) || q.tgt(kn1,2)~=0,               %if FC trial then save force data
                            if tvxmax > wsize, % time of max x velocity is more than windowsize
                                if tvxmax+wsize > length(fyr),
                                    ...
                                    dat.axr{kn}(:,ks)=nan(2*wsize+1,1);
                                    dat.ayr{kn}(:,ks)=nan(2*wsize+1,1);
                                    dat.axr{kn}(1:wsize*2+1-Nmissing,ks)=axr(tvxmax-wsize:end);
                                    dat.ayr{kn}(1:wsize*2+1-Nmissing,ks)=ayr(tvxmax-wsize:end);
                                else
                                    ...
                                    dat.axr{kn}(:,ks) = axr(tvxmax + [-wsize:wsize]);
                                    dat.ayr{kn}(:,ks) = ayr(tvxmax + [-wsize:wsize]);
                                ...
                                if tvxmax+wsize > length(fyr)
                                    ...
                                    dat.axr{kn}(:,ks)=nan(2*wsize+1,1);
                                    dat.ayr{kn}(:,ks)=nan(2*wsize+1,1);
                                    dat.axr{kn}(wsize+2-tvxmax:wsize+1-tvxmax+Ndata,ks)=axr;
                                    dat.ayr{kn}(wsize+2-tvxmax:wsize+1-tvxmax+Ndata,ks)=ayr;
                                    ...
                                    dat.axr{kn}(1:padsize,ks)=NaN*ones(padsize,1);
                                    dat.ayr{kn}(1:padsize,ks)=NaN*ones(padsize,1);
                                    dat.axr{kn}((padsize+1):(wsize*2+1),ks) = axr(1:(tvxmax + wsize));
                                    dat.ayr{kn}((padsize+1):(wsize*2+1),ks) = ayr(1:(tvxmax + wsize));
                            ...
                            if tvxmax > wsize, % time of max x velocity is more than windowsize
                                if tvxmax+wsize > length(vxr)
                                    ...
                                    dat.axr{kn}(:,ks) = vxr(tvxmax + [-wsize:wsize]);
                                    dat.ayr{kn}(:,ks) = vyr(tvxmax + [-wsize:wsize]);
