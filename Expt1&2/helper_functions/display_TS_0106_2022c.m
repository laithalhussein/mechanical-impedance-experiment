...
fc_size = tgt_tmp(test1_idx(fc_pert),end);
...
plot(fc_pert, fc_size, 'color', 'b', 'linestyle', 'none', 'marker', 'p', 'markersize', 12);
...
%% Experiment 1 training schedules (make sure modeling code is already run in the main loop)

%random FC expt
train_trials = [1:Nm];

figure; hold on;
plot(train_trials, ierr_tmp2, 'color', 'b', 'markersize', 6);
ylim([-8.5, 8.5]);
ax = gca;
ax.YTick = [-8, 0, 8];
ax.XTick = [0, Nm];
ylabel('Imposed FC error (deg)');
xlabel('Trial number');
title('Random FC Expt');


%random FF Expt
Btmp = FF_mfa.FFc_size;
figure; hold on;
plot(train_trials, Btmp, 'color', 'r', 'markersize', 6);
ylim([-10,10]);
ax = gca;
ax.YTick = [-10, 0, 10];
ax.XTick = [0, Nm];
