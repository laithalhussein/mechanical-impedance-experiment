function [dat,info]=run_basic_crunch_hse_shooting_1206_2021a(dir1)
...
%determine maximum number of trials across subjects
nmax = sum(info.blocksize);

%pre-allocate some arrays
dat.mtime_real = nan(nmax, length(info.sublist));
dat.rt = nan(nmax, length(info.sublist));
...
for kb = 1:length(info.blocksize), disp(['Block ',info.blockletter(kb)]); % block counter
    for ks = 1:length(info.sublist), disp([info.sublist{ks},' ',num2str(toc)]) % subject counter
        ...
        if 1, %~isfield(q,'tgt'),
            ...
            %commenting the below line, since participants can have different tgt files
            %q.tgt = tgt_all(cs(kb)+1:cs(kb+1),:);
            
            sub_tgt_tmp = tgt_dat(ks,:);
            sub_tgt_all = vertcat(sub_tgt_tmp{:});
            
            q.tgt = sub_tgt_all(cs(kb)+1:cs(kb+1),:);
        ...
        for kn1=2:info.blocksize(kb), %The first 'movement' is just to get to the initial target.
            kn=kn1+sum(info.blocksize(1:kb-1)); %if ks == 3 && kn == 2295, keyboard, end
            if length(q.rawdata)>=kn1,
                if ~isempty(q.rawdata{kn1}) && size(q.rawdata{kn1},1)>(n1+10)
                    ...
                    % Compute the movement times according to the movement
                    % stages.  This gives a reliable measure of how long
                    % the robot thought the movement took (helpful for
                    % calculating whether rewards were given).
                    ...
                    % They are allowed to go starting at movement stage 3.
                    % However, stage 3 gets skipped if they go too early
                    % (set RT=0 in this case).
                    % Movement stage 4 starts when they are going.
                    if sum(mov_stage==3)==0
                        rt = 0;
                    elseif sum(mov_stage==4)==0
                        rt = nan;
                    else
                        rt = find(mov_stage==4,1,'first')-find(mov_stage==3,1,'first');
                    end
                    dat.rt(kn,ks) = rt*0.005;
                    ...
                    if  dat.good(kn,ks),
                        ...
                        if q.tgt(kn1,7) || q.tgt(kn1,2)~=0,               %if FC trial then save force data
                            if tvxmax > wsize, % time of max x velocity is more than windowsize
                                if tvxmax+wsize > length(fyr),
                                    ...
                                    dat.ms{kn}(1:wsize*2+1-Nmissing,ks)=mov_stage(tvxmax-wsize:end);
                                else
                                ...
                                if tvxmax+wsize > length(fyr)
                                    ...
                                    dat.ms{kn}(wsize+2-tvxmax:wsize+1-tvxmax+Ndata,ks)=mov_stage;
                                    ...
                                    dat.ms{kn}(1:padsize,ks)=padms*ones(padsize,1);
                            ...
                            % NOTE: changed so that force data is padded with NaN's...
                            if tvxmax > wsize, % time of max x velocity is more than windowsize
                                ...
                                if tvxmax+wsize > length(vxr)
                                    ...
                                    dat.vyr{kn}(1:padsize,ks)=padvalvyr*ones(padsize,1);
                                    dat.pyr{kn}(1:padsize,ks)=padvalyr*ones(padsize,1);
                                    dat.ms{kn}(1:padsize,ks)=padvalms*ones(padsize,1);
                    ...
                    %Ok to pad these, since they correspond to passive movements
                    %Laith added
                    dat.fyr{kn}(:,ks)=nan(2*wsize+1,1);
                    dat.vxr{kn}(:,ks)=nan(2*wsize+1,1);
                    dat.vyr{kn}(:,ks)=nan(2*wsize+1,1);
