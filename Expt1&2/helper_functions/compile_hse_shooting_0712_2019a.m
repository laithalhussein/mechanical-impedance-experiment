...
vmax.pert.err_EC.all = vmax.all(:,idx.err_EC.all);
vmax.pert.err_FF.all = vmax.all(:,idx.err_FF.all);

vmax_FF_cat_all= reshape(vmax.all(:,idx.err_FF.all), [num_subjects*num_trip,1]);
vmax_FC_cat_all = reshape(vmax.all(:,idx.err_EC.all),[num_subjects*num_trip,1]);

%overlay the histograms
figure; hold on;
create_hist_1012_2018(vmax_FF_cat_all,'k',20);
create_hist_1012_2018(vmax_FC_cat_all,grey,15);
...
xpos_sub.err_FF.all = get_sub_data(xpos, repmat(idx.err_FF.all',num_subjects,1))*1000;
xpos_sub.err_EC.all = get_sub_data(xpos, repmat(idx.err_EC.all',num_subjects,1))*1000;

ypos_sub.err_FF.all = get_sub_data(ypos, repmat(idx.err_FF.all',num_subjects,1))*1000;
ypos_sub.err_EC.all = get_sub_data(ypos, repmat(idx.err_EC.all',num_subjects,1))*1000;

vel_sub.pert.err_FF.all = get_sub_data(vel, repmat(idx.err_FF.all',num_subjects,1));
vel_sub.pert.err_EC.all = get_sub_data(vel, repmat(idx.err_EC.all',num_subjects,1));

xp_FF_pa = nanmean(squeeze(nanmedian(xpos_sub.err_FF.all,2)),1); %population average
xp_EC_pa = nanmean(squeeze(nanmedian(xpos_sub.err_EC.all,2)),1);

xp_FF_std = nanstd(squeeze(nanmedian(xpos_sub.err_FF.all,2)),0,1);
xp_EC_std = nanstd(squeeze(nanmedian(xpos_sub.err_EC.all,2)),0,1);

%repeat for y-position
yp_FF_pa = nanmean(squeeze(nanmedian(ypos_sub.err_FF.all,2)),1); %population average
yp_EC_pa = nanmean(squeeze(nanmedian(ypos_sub.err_EC.all,2)),1);

yp_FF_std = nanstd(squeeze(nanmedian(ypos_sub.err_FF.all,2)),0,1);
yp_EC_std = nanstd(squeeze(nanmedian(ypos_sub.err_EC.all,2)),0,1);

%get average velocity
vel_FF_pa = nanmean(squeeze(nanmedian(vel_sub.pert.err_FF.all,2)),1); %population average
vel_EC_pa = nanmean(squeeze(nanmedian(vel_sub.pert.err_EC.all,2)),1);
...
%% save the end movement data by sign

idx.err_EC.P = idx.err_EC.all(find(err_sign.EC(1,:)==1));
idx.err_EC.N = idx.err_EC.all(find(err_sign.EC(1,:)==-1));

idx.err_FF.P = idx.err_FF.all(find(err_sign.FF(1,:)==-1));
idx.err_FF.N = idx.err_FF.all(find(err_sign.FF(1,:)==1));

xp_end.FC.all = get_sub_data(xp_end_tmp, repmat(idx.err_EC.all',num_subjects,1));
yp_end.FC.all = get_sub_data(yp_end_tmp, repmat(idx.err_EC.all',num_subjects,1));

xp_end.FC.P = get_sub_data(xp_end_tmp, repmat(idx.err_EC.P',num_subjects,1));
yp_end.FC.P = get_sub_data(yp_end_tmp, repmat(idx.err_EC.P',num_subjects,1));

xp_end.FC.N = get_sub_data(xp_end_tmp, repmat(idx.err_EC.N',num_subjects,1));
yp_end.FC.N = get_sub_data(yp_end_tmp, repmat(idx.err_EC.N',num_subjects,1));

%save data data by sign as well
f_sub.pre.err_FF.P = get_sub_data(fr, repmat(idx.err_FF.P'-1,num_subjects,1));
f_sub.pre.err_EC.P = get_sub_data(fr, repmat(idx.err_EC.P'-1,num_subjects,1));

f_sub.post.err_FF.P = get_sub_data(fr, repmat(idx.err_FF.P'+1,num_subjects,1));
f_sub.post.err_EC.P = get_sub_data(fr, repmat(idx.err_EC.P'+1,num_subjects,1));

f_sub.pre.err_FF.N = get_sub_data(fr, repmat(idx.err_FF.N'-1,num_subjects,1));
f_sub.pre.err_EC.N = get_sub_data(fr, repmat(idx.err_EC.N'-1,num_subjects,1));

f_sub.post.err_FF.N = get_sub_data(fr, repmat(idx.err_FF.N'+1,num_subjects,1));
f_sub.post.err_EC.N = get_sub_data(fr, repmat(idx.err_EC.N'+1,num_subjects,1));

f_sub.pert.err_FF.P = get_sub_data(fr, repmat(idx.err_FF.P',num_subjects,1));
f_sub.pert.err_EC.P = get_sub_data(fr, repmat(idx.err_EC.P',num_subjects,1));

f_sub.pert.err_FF.N = get_sub_data(fr, repmat(idx.err_FF.N',num_subjects,1));
f_sub.pert.err_EC.N = get_sub_data(fr, repmat(idx.err_EC.N',num_subjects,1));

f_sub.pert.err_FF.all = get_sub_data(fr, repmat(idx.err_FF.all',num_subjects,1));
f_sub.pert.err_EC.all = get_sub_data(fr, repmat(idx.err_EC.all',num_subjects,1));

%velocity
vel_sub.pre.err_FF.P = get_sub_data(vel, repmat(idx.err_FF.P'-1,num_subjects,1));
vel_sub.pre.err_EC.P = get_sub_data(vel, repmat(idx.err_EC.P'-1,num_subjects,1));

vel_sub.post.err_FF.P = get_sub_data(vel, repmat(idx.err_FF.P'+1,num_subjects,1));
vel_sub.post.err_EC.P = get_sub_data(vel, repmat(idx.err_EC.P'+1,num_subjects,1));

vel_sub.pre.err_FF.N = get_sub_data(vel, repmat(idx.err_FF.N'-1,num_subjects,1));
vel_sub.pre.err_EC.N = get_sub_data(vel, repmat(idx.err_EC.N'-1,num_subjects,1));

vel_sub.post.err_FF.N = get_sub_data(vel, repmat(idx.err_FF.N'+1,num_subjects,1));
vel_sub.post.err_EC.N = get_sub_data(vel, repmat(idx.err_EC.N'+1,num_subjects,1));

vel_sub.pert.err_FF.P = get_sub_data(vel, repmat(idx.err_FF.P',num_subjects,1));
vel_sub.pert.err_EC.P = get_sub_data(vel, repmat(idx.err_EC.P',num_subjects,1));

vel_sub.pert.err_FF.N = get_sub_data(vel, repmat(idx.err_FF.N',num_subjects,1));
vel_sub.pert.err_EC.N = get_sub_data(vel, repmat(idx.err_EC.N',num_subjects,1));

%xp
xpos_sub.pre.err_FF.P = get_sub_data(xpos, repmat(idx.err_FF.P'-1,num_subjects,1));
xpos_sub.pre.err_EC.P = get_sub_data(xpos, repmat(idx.err_EC.P'-1,num_subjects,1));

xpos_sub.post.err_FF.P = get_sub_data(xpos, repmat(idx.err_FF.P'+1,num_subjects,1));
xpos_sub.post.err_EC.P = get_sub_data(xpos, repmat(idx.err_EC.P'+1,num_subjects,1));

xpos_sub.pre.err_FF.N = get_sub_data(xpos, repmat(idx.err_FF.N'-1,num_subjects,1));
xpos_sub.pre.err_EC.N = get_sub_data(xpos, repmat(idx.err_EC.N'-1,num_subjects,1));

xpos_sub.post.err_FF.N = get_sub_data(xpos, repmat(idx.err_FF.N'+1,num_subjects,1));
xpos_sub.post.err_EC.N = get_sub_data(xpos, repmat(idx.err_EC.N'+1,num_subjects,1));

xpos_sub.pert.err_FF.P = get_sub_data(xpos, repmat(idx.err_FF.P',num_subjects,1));
xpos_sub.pert.err_EC.P = get_sub_data(xpos, repmat(idx.err_EC.P',num_subjects,1));

xpos_sub.pert.err_FF.N = get_sub_data(xpos, repmat(idx.err_FF.N',num_subjects,1));
xpos_sub.pert.err_EC.N = get_sub_data(xpos, repmat(idx.err_EC.N',num_subjects,1));

%yp
ypos_sub.pre.err_FF.P = get_sub_data(ypos, repmat(idx.err_FF.P'-1,num_subjects,1));
ypos_sub.pre.err_EC.P = get_sub_data(ypos, repmat(idx.err_EC.P'-1,num_subjects,1));

ypos_sub.post.err_FF.P = get_sub_data(ypos, repmat(idx.err_FF.P'+1,num_subjects,1));
ypos_sub.post.err_EC.P = get_sub_data(ypos, repmat(idx.err_EC.P'+1,num_subjects,1));

ypos_sub.pre.err_FF.N = get_sub_data(ypos, repmat(idx.err_FF.N'-1,num_subjects,1));
ypos_sub.pre.err_EC.N = get_sub_data(ypos, repmat(idx.err_EC.N'-1,num_subjects,1));

ypos_sub.post.err_FF.N = get_sub_data(ypos, repmat(idx.err_FF.N'+1,num_subjects,1));
ypos_sub.post.err_EC.N = get_sub_data(ypos, repmat(idx.err_EC.N'+1,num_subjects,1));

ypos_sub.pert.err_FF.P = get_sub_data(ypos, repmat(idx.err_FF.P',num_subjects,1));
ypos_sub.pert.err_EC.P = get_sub_data(ypos, repmat(idx.err_EC.P',num_subjects,1));

ypos_sub.pert.err_FF.N = get_sub_data(ypos, repmat(idx.err_FF.N',num_subjects,1));
ypos_sub.pert.err_EC.N = get_sub_data(ypos, repmat(idx.err_EC.N',num_subjects,1));
...
%try it on an individual subject-basis [b,err_norm_cut,num_rem, xc]
[b_est.tst1, err_norm_cut.tst1, num_rem.tst1, xc.tr1.all] = determine_triplet_LR_0628_2019a(ILF_training1_cut, norm_err.tr1, p_all.tr1(:,1) );

%% estimate learning rate on test1 data

[b_est.tst1, err_norm_cut.tst1, num_rem.tst1] = determine_triplet_LR_0628_2019a(ILF_test1, norm_err.tst1, p_all.tst1(:,1) );
