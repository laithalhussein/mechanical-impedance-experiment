...
for kq=1:num_subjects
    ...
    %determine the "imposed" error during FF on a per-subject basis
    ierr_test2_sub(ierr_test2_FF_idx) = err_FF_all_sub_sign(kq,:);
...
%[b_est.avg.tr1, err_norm_cut.avg.tr1, num_rem.avg.tr1, xc.tr1] = determine_triplet_LR_avg(res_tr1, ierr_tr1_norm, p_tr1(1) ); %ILF_training1_cut
[b_est.avg.tr1, err_norm_cut.avg.tr1, num_rem.avg.tr1, xc.tr1] = determine_triplet_LR_avg(ILF_avg_training1, ierr_tr1_norm, p_tr1(1) );

figure;
plot(err_norm_cut.avg.tr1, xc.tr1, '.');

LRi.tr1 = regress(xc.tr1', err_norm_cut.avg.tr1); %learning rate from individual analysis
...
[b_est.avg.tst1, err_norm_cut.avg.tst1, num_rem.avg.tst1, xc.tst1] = determine_triplet_LR_avg(ILF_avg_test1, ierr_tst1_norm, p_tst1(1), b_stiff_tst1*EC_err_mm, ILF_uf_avg.ar.err_EC, res_tst1  );
%[b_est_ns.avg.tst1, err_norm_cut.avg.tst1, num_rem.avg.tst1, xc.tst1] = determine_triplet_LR_avg_ns(res_tst1, ierr_tst1_norm, p_tst1(1), b_stiff_tst1*EC_err_mm, ILF_uf_avg.ar.err_EC, res_tst1  );
...
%%%% calculate average learning rate for trials with the EC error
 ...
 ECp_err_idx = find( ierr_test1(2:end-num_rem.tst1) == EC_err_mm);
 ECn_err_idx = find( ierr_test1(2:end-num_rem.tst1) == -EC_err_mm);

 nanmean(b_est.avg.tst1(ECp_err_idx))
 nanmean(b_est.avg.tst1(ECn_err_idx))
 
%EC_err_idx = find( ierr_test1(2:end-num_rem.tst1) == EC_err_mm);

% for kk=1:num_subjects
%     b_est.tst1(kk,err_norm_cut.tst1(kk,:)<0) = b_est.tst1(kk,err_norm_cut.tst1(kk,:)<0) * -1;
% end

% b_est.avg.tst1(:,err_norm_cut.avg.tst1<0) = b_est.avg.tst1(err_norm_cut.avg.tst1<0) * -1;
...
%try estimating the learning based on the averaged data
[b_est.avg.tst2, err_norm_cut.avg.tst2] = determine_triplet_LR_avg(ILF_avg_test2, ierr_tst2_norm, p_tst2(1));

%dont count errors that are less than the cutoff
% small_FF_err_idx = find(abs(err_norm_cut.avg.tst2) < err_cutoff);
% 
% b_est.avg.tst2(small_FF_err_idx) = NaN;

%flip the learning rates where appropriate
%b_est.avg.tst2(err_norm_cut.avg.tst2 < 0)  = b_est.avg.tst2(err_norm_cut.avg.tst2 < 0) * -1;
nanmean(b_est.avg.tst2(ierr_test2_FF_idx-num_rem.tst2))

%%%% calculate average learning rate for trials with the FF error
% est_ar_FF = b_est.avg.tst2(ierr_test2_FF_idx-num_rem.tst2);

%look closely at the AR to FF trials
%nanmean(err_FF_all_sub_sign,1) * b_stiff_tst2;
% err_norm_cut.avg.tst2(ierr_test2_FF_idx-num_rem.tst2)

%flip the learning rates where appropriate
for kk=1:num_subjects
    b_est.tst2(kk,err_norm_cut.tst2(kk,:)<0) = b_est.tst2(kk,err_norm_cut.tst2(kk,:)<0) * -1;
end
...
[b_est.avg.tr2, err_norm_cut.avg.tr2, num_rem.avg.tr2, xc.tr2] = determine_triplet_LR_avg(ILF_training2_cut, ierr_tr2_norm, p_tr2(1) );
...
nanmean(nanmean(b_est.tst2(:,ierr_test2_FF_idx-num_rem.tst2),2))

%adaptive response measured from data with decay added
nanmean(nanmean(ILF.decay.err_FF,2) + nanmean(ILF.ar.err_FF,2) )
