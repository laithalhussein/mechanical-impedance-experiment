function [dat,info]=run_basic_crunch_varec(dir1)
        ...
        for kn1=1:info.blocksize(kb),
            ...
            if length(q.rawdata)>=kn1,
                if ~isempty(q.rawdata{kn1})
                    ...
                    if firstnonzero+10>length(fx), firstnonzero = []; end %Andrew added this line to handle an exception.  It doesn't change fx, fy this way.
                    fx(1:firstnonzero)=nanmean(fx((firstnonzero+1):(firstnonzero+10)));
                    firstnonzero=find(fy~=0,1);
                    if firstnonzero+10>length(fy), firstnonzero = []; end %Andrew added this line to handle an exception.  It doesn't change fx, fy this way.
                    ...
                    if max(abs(vxr))>vmaxthresh | max(abs(vxr))<vminthresh
                        dat.good(kn,ks)=0;
