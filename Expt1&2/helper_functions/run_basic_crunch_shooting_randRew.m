        ...
        for kn1=2:info.blocksize(kb), %The first 'movement' is just to get to the initial target.
            ...
            if length(q.rawdata)>=kn1,
                if ~isempty(q.rawdata{kn1}) && size(q.rawdata{kn1},1)>(n1+10)
                    ...
                    mvt_stage = q.rawdata{kn1}(n1:end,14);
                    iRew = find(mvt_stage==6,1,'first');
                    rew = q.rawdata{kn1}(n1+iRew,15);
                    
                    % buffer fx and fy
                    firstnonzero=find(fx(1:iRew)~=0,1);
                    fx(1:firstnonzero)=nanmean(fx((firstnonzero+1):(firstnonzero+10)));
                    firstnonzero=find(fy(1:iRew)~=0,1);
                    ...
                    elseif isempty(iRew), dat.good(kn,ks) = 0; % They didn't make a normal movement -- never got to movement stage 6.
