function [dat,info]=run_basic_crunch_hse_shooting(dir1)
...
if length(dt) > 0,
    ...
    %detect what kind of experiment sequence was conducted
    %to do this, look at the very first training trial
    %the possibilities are VZL, ZLV, and LVZ
    for k1c=1:length(info.sublist)
       first_training_block = tgt_dat{k1c,4};
      
       if first_training_block(2,7)==2 & first_training_block(2,end)~=0 %then we have a vEC
           info.exp_seq{k1c} = ['VZL'];
           
       elseif first_training_block(2,7)==2 & first_training_block(2,end)==0 %then its a zEC
           info.exp_seq{k1c} = ['ZLV'];
           
       else %otherwise we start with a low-stiffness trial (null)
           info.exp_seq{k1c} = ['LVZ'];
       end 
    end
