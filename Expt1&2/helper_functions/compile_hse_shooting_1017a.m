...
%NOTE: Its probably OK to use test1 as part of the mean subtraction
...
if mean_subtract_flag
    ...
    %subtract the mean response for the AR too (this whole thing should be handled way ealier)
    ILF_uf.ar.err_EC = bsxfun(@(x,y) x-y, ILF_uf.ar.err_EC, nanmean(ILF_training1_cut_raw,2));
    ILF.ar.err_EC = bsxfun(@(x,y) x-y, ILF.ar.err_EC, nanmean(ILF_training1_cut_raw,2));
    
    ILF_uf.pre.err_EC = bsxfun(@(x,y) x-y, ILF_uf.pre.err_EC, nanmean(ILF_training1_cut_raw,2));
    ILF.pre.err_EC = bsxfun(@(x,y) x-y, ILF.pre.err_EC, nanmean(ILF_training1_cut_raw,2));
...
tst1_fit = stiffness_model2(p_tst1, ierr_test1);
