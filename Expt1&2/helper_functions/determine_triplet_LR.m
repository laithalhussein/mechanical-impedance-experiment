function [b,err_norm_cut,num_rem, xc] = determine_triplet_LR(x, err_norm, A)
...
for i=1:num_subjects
    ...
    %     subplot(1,num_subjects,i); hold on;
    %     plot(err_norm(i,2:end-num_rem),b(i,:),'.'); %plot by normalized error
    %
    %     ylabel('Learning rate');
    %     xlabel('normalized error (mm)');
    
%     xc(i,:) = x_corrected(2:end);
%     xu(i,:) = x_uncorrected;
    
    overall_LR = regress(xc', err_norm_cut);
