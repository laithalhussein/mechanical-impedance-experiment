...
for k1=1:num_subjects
    ...
    idx.FF.all = triplets_tmp;
    idx.FF.vEC = vEC_triplets_tmp;
    idx.FF.zEC = zEC_triplets_tmp;
    idx.FF.null = null_triplets_tmp;
    ...
    %get all vEC trials (%make sure to catch the last trial during the vEC training period)
    vEC_test_tmp = find(sub_tgt_all_fwrd(:,7) > 0 & sub_tgt_all_fwrd(:,end) ~= 0); 
    vEC_test_tmp = vEC_test_tmp(vEC_test_tmp>idx.training.vEC.all(k1,end));
    vEC_tmp = [idx.training.vEC.all(k1,:), vEC_test_tmp'];
    idx.vEC.all(k1,:) = vEC_tmp;
    
    %we also want to know the indices of the refresher trials during the test period
    idx.refresh.vEC(k1,:) = vEC_tmp(vEC_tmp > idx.training.vEC.all(k1,end) & vEC_tmp <= idx.test.vEC.all(k1,end) );
    
    %we need to know the beginning of each set
    idx.refresh.start(k1,:) = [idx.refresh.vEC(k1,1), idx.refresh.vEC(k1, find( diff(idx.refresh.vEC(k1,:)) > 1) + 1)];
...
for k2=1:num_subjects
    ...
    for k3=2:2:total_trials
        ...
        pillow_idx = find( dat.pxr{k3}(:,k2) > 0.1, 1, 'first') - 1; %should it be 11?
...
max_pos = nan(num_subjects, total_trials/2);
...
for k2=1:num_subjects
    ...
    for k3=2:2:total_trials
        ...
        %take the average of the last 20 samples to see how far past the pillow people are landing
        max_pos(k2,cc) = nanmean(dat.pxr{k3}(end-19:end,k2));
...
for kq=1:num_subjects
    ...
    ACI.ar.vEC(kq,:) = ( ACI.post.vEC(kq,:) - ACI.pre.vEC(kq,:) ) .* -FF_sign.vEC(kq,:);
    ACI.ar.zEC(kq,:) = ( ACI.post.zEC(kq,:) - ACI.pre.zEC(kq,:) ) .* -FF_sign.vEC(kq,:);
    ACI.ar.null(kq,:) = ( ACI.post.null(kq,:) - ACI.pre.null(kq,:) ) .* -FF_sign.vEC(kq,:);
    ...
    %ACI.refresh.vEC = ACI_all(kq, idx.refresh.vEC(kq,:) );
...
burn_trials = 30; %a certain number of trials need to be removed to exclude the effects of the EC bias
ACI_training_vEC_cut_raw = ACI.training.vEC(:, [burn_trials+1:end]);


%subtract each participant's mean response to mitigate EC bias
EC_bias_flag = 1;
if EC_bias_flag
   ACI_training_vEC_cut = bsxfun(@(x,y) x-y, ACI_training_vEC_cut_raw, nanmean(ACI_training_vEC_cut_raw,2));
    EC_bias_flag = 0;
end
...
ACI_training_vEC_cat = reshape(ACI_training_vEC_cut', [size(ACI_training_vEC_cut,2) * size(ACI_training_vEC_cut,1), 1]);
...
X1 = [ierr_training_rep];
[b2,b2int,~,~,s2] = regress(ACI_training_vEC_cat, X1);

ACI_avg_training_vEC = nanmean(ACI_training_vEC_cut, 1);

figure; hold on;
plot(ierr_training_rep, ACI_training_vEC_cat, '.');
%axis([-10, 10, -2.5, 2.5]);
...
corr(ierr_training_rep, ACI_training_vEC_cat, 'rows', 'complete')

corr(ierr_training_cut, ACI_avg_training_vEC')

axis([-10, 10, -1, 1]);

%% estimate effect of stiffness through regression

qq = nlinfit(ierr_training_cut, ACI_avg_training_vEC, @stiffness_model, [0.8,0.07,0.07]);

% stiffness_effect = (0*ACI_avg_training_vEC+b2(1)) + (ierr_training_cut*b2(2))';
stiffness_effect = (ierr_training_cut*b2(1))';

%%% remove effects of stiffness
ACI_avg_training_ns = ACI_avg_training_vEC -  stiffness_effect; %offset is first in b2

%fit the model
[qq2,r2,J2] = nlinfit(ierr_training_cut, ACI_avg_training_ns', @stiffness_model2, [0.8,0.07,0.1]);
cc2 = nlparci(qq2,r2,'Jacobian',J2);

%fit it onto a model that saturates with error size
qq3 = nlinfit(ierr_training_cut, ACI_avg_training_ns', @stiffness_model_sat, [0.8, 0.07, 5, 0.1] );
...
%% manually check the learning rate during the training period

%look for compatible errors

% figure; hold on;
% plot(ierr_training_cut);
% ylabel('Errors (mm)'); xlabel('Trial #');
% 
% %try 23, 24, and 25
% 
% tri1 =[23:25];
% 
% %ar1 = ACI_avg_training_ns(tri1(end)) - ACI_avg_training_ns(tri1(1));
% ilf1 = ACI_avg_training_ns(tri1(2)) * b2(2); %fraction of ideal force accounted for
% ideal_f1 = squeeze(nanmean(vel_training_vEC_cut(:,tri1(2),:),1)) * B;



%% analyze the AC from the refresher trials

%note: only 2 refresher trials immediately follow block breaks...

%optionally want to remove the first x trials from each set
num_rem_trials = [0,1,2,3]; %how many trials in each refresher set to remove

params1 = cell(1,length(num_rem_trials));
ci1 = cell(1,length(num_rem_trials));

stiffness_effect2 = NaN(length(num_rem_trials),1);

elr = NaN(length(num_rem_trials),1); %estimated learning rate

for kk=1:length(num_rem_trials)
    
    if num_rem_trials(kk)>0
        
        idx_rem_trials = NaN(size(idx.refresh.start,1), size(idx.refresh.start,2) * num_rem_trials(kk));
        rem_trials_tmp = cell(size(idx.refresh.start,1), size(idx.refresh.start,2) );
        
        updated_refresh_idx = NaN(size(idx.refresh.vEC,1), size(idx.refresh.vEC,2) - ...
            (size(idx.refresh.start,2) * num_rem_trials(kk))); %this will hold the refresh trials after the ones we dont want are removed
        
        for kq1=1:num_subjects
            
            for kq2 = 1: size(idx.refresh.start,2)
                rem_trials_tmp{kq1,kq2} = [idx.refresh.start(kq1, kq2) : idx.refresh.start(kq1, kq2) + num_rem_trials(kk) - 1];
            end
            
            idx_tmp = horzcat(rem_trials_tmp{kq1,:});
            idx_rem_trials(kq1,:) = idx_tmp;
            
            updated_refresh_idx_tmp = find( ~ismember( idx.refresh.vEC(kq1,:), idx_rem_trials(kq1,:)) ); %the trials that we want to keep
            
            updated_refresh_idx(kq1,:) = idx.refresh.vEC(kq1, updated_refresh_idx_tmp);
            
            ACI_refresh_vEC{kk}(kq1,:) = ACI_all(kq1, updated_refresh_idx(kq1,:) );
            
        end
    else
        
        for kq1 = 1:num_subjects
            ACI_refresh_vEC{kk}(kq1,:) = ACI_all(kq1, idx.refresh.vEC(kq1,:) );
            updated_refresh_idx = idx.refresh.vEC;
        end
        
    end
    
    %subtract the mean response for the refresher trials as well, but still use the data from the training period (we dont know how the triplets might
    %contaminate the response of the refresher trials)
    bias_flag2 = 1;
    if bias_flag2
        ACI_refresh_all = bsxfun(@(x,y) x-y, ACI_refresh_vEC{kk}, nanmean(ACI_training_vEC_cut_raw, 2));
        bias_flag2 = 0;
    end
    
    %take the average
    ACI_avg_refresh = nanmean(ACI_refresh_all,1);
    
    %get the errors administered for these refresher trials
    ierr_refresh_deg = tgt_all{1}(updated_refresh_idx(1,:), end); %this is in degrees
    ierr_refresh  = tand(ierr_refresh_deg) *100; %in mm
    
    
    %restimate the effect of stiffness, including both the training and test data
    stiffness_effect2(kk) = regress( [ACI_avg_training_vEC'; ACI_avg_refresh'], [ierr_training_cut; ierr_refresh]);
    
    %subtract the effect of stiffness
%     ACI_avg_refresh_ns = ACI_avg_refresh - (ierr_refresh * b2(1))';
    ACI_avg_refresh_ns = ACI_avg_refresh - (ierr_refresh * stiffness_effect2(kk))';
    
    %initialiaze a guess for the parameter D in the model
    start_idx_tmp = [1, find( diff(idx.refresh.vEC(1,:)) > 1) + 1];
    D_initial = zeros(length(ierr_refresh),1);
    D_initial(start_idx_tmp) = 0.1;
    
    %fit the model
     [qq4,r4,J4] = nlinfit(ierr_refresh, ACI_avg_refresh_ns', @stiffness_model_refresh, [0.7;0.07;D_initial]);
    %[qq4,r4,J4] = nlinfit({ierr_refresh, start_idx_tmp}, ACI_avg_refresh_ns', @stiffness_model_refresh2, [0.7;0.07;start_idx_tmp'*0+0.1]);
    cc4 = nlparci(qq4,r4,'Jacobian',J4);
    
    params1{kk} = qq4;
    ci1{kk} = cc4;
    
    elr(kk) = qq4(2) / stiffness_effect2(kk);
    
    
    if num_rem_trials(kk)==0
        
        %%%% make sure the first trial in each set isnt systematically different
        
        %start_idx_tmp = [1, find( diff(idx.refresh.vEC(1,:)) > 1) + 1];
        
        
        figure; hold on;
        plot(ACI_avg_refresh_ns);
        plot(start_idx_tmp, ACI_avg_refresh_ns(start_idx_tmp), 'o', 'markersize',8,'color','r');
        title('Average ILF during refresher trials');
...
FF_err_all_tmp = abs(err(:,idx.FF.all)); %dont care about direction
FF_err_all = reshape(FF_err_all_tmp,[size(FF_err_all_tmp,1)*size(FF_err_all_tmp,2),1]);

FF_err_vEC_tmp = abs(err(:,idx.FF.vEC));
FF_err_vEC = reshape(FF_err_vEC_tmp,[size(FF_err_vEC_tmp,1)*size(FF_err_vEC_tmp,2),1]);

%%%% looks like we get an average displacement ~16-18 mm, corresponding to ~8.5-10 degrees
